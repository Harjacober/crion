...
static Token blockComment() {
    while (!(peek(scanner) == '*' && peekNext(scanner) == '/') && !isAtEnd) {
        advance(scanner);
    }
    if (isAtEnd) {
        return errorToken("unterminated comment");
    }
    //double advance past the characters * and / end a comment
    advance();
    advance();
...
static TokenType identifierType() {
    switch (scanner.start[0]) {
        ...
        case 'f':
            ...
            break;
        case 'e': {
            //match else and elseif
            int token = checkKeyword(1, 3, "lse", TOKEN_ELSE);
            if (token == TOKEN_ELSE && scanner.current - scanner.start > 4) {
...
Token scanToken() {
    ...
    switch (c)
    {
        ...
        default:
            ...
            return errorToken("Unexpected chracter.");
    }
}